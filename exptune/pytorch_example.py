...
class PytorchMnistMlpConfig(ExperimentConfig):
    ...
    def search_strategy(self):
        # These are alternative strategies that can be imported.
        # return RandomSearchStrategy(num_samples=5)
        # return GridSearchStrategy({"lr": 5, "dropout": 5}) # batch size is implicit
        return BayesOptSearchStrategy(
            num_samples=25, max_concurrent=4, metric=self.trial_metric()
        )
    ...
    def trial_metric(self):
        return Metric("val_accuracy", "max")
